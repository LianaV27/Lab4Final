...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="post-page-content">
		...
		$graceful_defaults = array(
			'before' => '<p class="site-pagination">' . esc_html__( 'Pages:', 'graceful-chic-blog' ),
			'after' => '</p>'
		);
	...
	<footer class="post-footer">
		...
		if ( graceful_options( 'single_post_page_show_comments' ) && comments_open() ) {
			comments_popup_link( esc_html__( 'No Comments', 'graceful-chic-blog' ), esc_html__( '1 Comment', 'graceful-chic-blog' ), '% ' . esc_html__( 'Comments', 'graceful-chic-blog' ), 'post-comments');
		}
		?>
	</footer>
